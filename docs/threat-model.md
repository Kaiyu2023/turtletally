...
- Compromised browser content, uploaded files, statement rows, dependencies, build tooling, or conversational context.
...
- Local deployment tooling, GitHub pull requests and workflows, and the AWS control plane.
...
| Administrator or CI to AWS   | Infrastructure changes, restore, and revocation actions     | MFA-backed SSO or narrowly bound OIDC, human gates, explicit context, reviewed `cdk diff`, CloudTrail |
...
| An administrator, deployment, or dependency introduces excessive access, data loss, or credential exposure                  | No root or IAM-user keys; MFA-backed SSO; narrowly scoped runtime and deployment roles; pinned dependencies and Actions; local audits and secret scan; explicit stage/account/region/stack; reviewed `cdk diff`; no direct production destruction                      | The single AWS account and owner recovery path remain concentrated trust points. CloudTrail, access review, offline recovery material, and independent phishing-resistant factors reduce but do not remove that risk. |
...
Any failed compatibility proof, high or critical dependency finding, unreviewed infrastructure diff, unavailable recovery path, or unexpected paid service is a stop condition.
