...
- The owner, using a browser, one or more MCP-capable assistants, and approved recovery procedures.
- Cognito, CloudFront, API Gateway, Lambda, DynamoDB, S3, KMS, EventBridge Scheduler, and supporting AWS services.
- An MCP client acting for the owner under a scoped, audience-bound access token (ADR 0011).
...
| Assistant to MCP API                | Tool inputs and an OAuth bearer token                                 | Dedicated domain and per-assistant client, published protected-resource metadata, issuer/expiry/audience/scope checks, host allowlist  |
...
| A client presents another assistant's token, replays an MCP token, broadens scope, injects instructions, or retries a write            | Dedicated domain and one OAuth client per assistant, each revocable on its own; issuer, expiry, audience, and scope checks on every request; explicit accepted-audience list with no wildcard; host allowlist; bounded action-specific tools; preview and commit; single-use proposals; no generic query, patch, or delete tool                                                                  | ADR 0011 drops client certificates as a requirement, so a stolen token is refused by binding and revocation rather than by the network. Cognito resource binding is unproven until the compatibility test. Each assistant's handling of returned data is outside this system and needs owner review.  |
...
| Logs, metrics, audit events, or support evidence leak financial content                                                                | Structured coarse events; request identifiers; payload and secret redaction; audit field and version metadata instead of values; keyed canonical before/after digests with a protected, rotatable audit key only where correlation is required; bounded retention; synthetic diagnostics                                                                                                         | Reduced edge logging on an eligible CloudFront flat-rate plan limits investigation detail. Application audit coverage and alarm evidence must compensate for that blind spot.                                                                                                                         |
...
- Each connected assistant's retention, training, sharing, and administrator settings.
- The proven Cognito resource binding, OAuth callback, and MCP audience and scope behaviour, per assistant.
